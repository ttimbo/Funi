...
"---" Separates context and/or explanations from the details of a scene.

Specific details are described in scene entries unless it provides important context.

**--EPISODE 1: STRONGHOLD FOX**
  [[EP1 Production]]
  [[Outline]]
  
  **Introduces Rossgivo, what Krushers are, the purpose of Strongholds, and how people adapted to living inside Stronghold Fox for the past 5 years without contact from the Outside World after the Fall of Chinelo.**
  
  The episode opens with an aerial wide shot of a large abandoned city with _Netherlands-style_ architecture, seemingly left to the natural elements for several years, as the setting sun creates a reddish evening sky.
  
  Far from its urban centre within its western outskirts, initially appears to be 6 wandering survivors, _Survey & Scavenging Team 1_, or SST 1 partaking on their _18th Expedition_ out from Stronghold Fox with the goal of reaching the suspected _New Frontlines_, believed to have been constructed to contain the breach of the original frontlines _5 years prior_, an event that left them isolated in Stronghold Fox with _no contact_ from the Outside World.
  
  SST 1 and their general position is brought to attention by a _civilian drone_ that flies past the camera above them, controlled by one of its members, _Dave Harris_, an autistic 24 year old Caucasian man with short trimmed brown hair, a small beard, who wears glasses to correct his nearsightedness.
  
  The drone has a dedicated controller with a built-in display, showing an aerial view of SST 1's surroundings through a small camera on the underside of the drone, and has an antenna attached onto it, which given the presence of a _No Internet_ icon in the top right of the display, and the small details around the USB-C port its connected to, seemed to be meant as an alternative option that was part of the drone's factory design.
  
  A brief exchange between Dave and Ruiz tells us that SST 1 is using the drone to look out for _something_, however it's only described as being absent from their surroundings, since there'd be no reason for them to specify further given its context.
  
  But presumably, Dave is referring to "_Krushers_", shown in the thumbnail: They have large red rectangular bodies, several metres in height, and about the width of a two-lane street, something that'd make them _rather conspicuous_ even if they're obscured by the environment.
  
  Also, the man in the military uniform standing off against the Krusher is _Ruiz Kurnell_.
  
  _Ruiz Kurnell_, a 24 year old Caucasian man with short brown hair wearing a grey camo military uniform with triangular insignias labeled "_40_", carefully peeks out from the alleyway SST 1 had been walking through, and looks both ways down the narrow two-lane street it leads out into, which he confirms is "_All Clear_" before quickly running across into a nearby _pedestrian street_, with the rest of SST 1 soon following behind him.
  ---
  This, paired with Dave's interaction with Ruiz, shows that Ruiz is the group's Leader.
  
  However, Ruiz lacks any other military equipment besides his uniform, which itself is battered with visible repair work in the form of additional stitching, and his behavior and treatment from the others shows that he's in the same predicament as them, rather than leading the group to an evacuation area or safe zone as an escort.
  ---
  The pedestrian street is lined with storefronts and a partially setup yet highly decorated festival, which a poster hung outside the street explains to be a celebration of the _1000 year anniversary_ of "_Rossgivo_": A fictional European nation similar in size and location to the Holy Roman Empire, which had existed since _January 1st, 1065_, and has a long history of militaristic victories; As is explained by a number of pamphlets and papers strewn along the street, handouts that were knocked over from their neat stacks by the wind at some point.
  
  Though the festival was likely meant for nearby residents with no need to be extravagant, it was still quite elaborate even in the state it was left in, with a fairly large number of tables setup along the street which have mostly withstood the natural elements, though many were left empty or weren't fully setup in time—Possibly due to whatever apocalyptic event had occurred soon after Rossgivo's 1000 year anniversary.
  
  By this point while SST 1 was walking down the street, Ruiz locates a small family-owned convenience store with what appears to be a 2 floor apartment on its upper floors, almost as if he was specifically looking out for it (the other stores SST 1 passed by weren't as valuable.)
  
  Ruiz attempts to open the front door of the store, but finds it to be locked.
  
  And so, Ruiz activates his _Arm Blades_, a custom-made weapon with multiple rows of blades on a large leather base that's strapped to Ruiz's right forearm, which he uses to break the front door's window so that he can open it from the inside.
  
  _SHOT: ARM BLADES_
   Ruiz's Arm Blades consist of 6 rows of interlocking triangular blades attached to strips of metal at their base, but is covered by long strips of leather that are bolted to them, with all 6 rows attached at their base to a much larger leather base with small hinge mechanisms, with the leather base being slightly less than the full length of Ruiz's right forearm, which it's strapped to with 2 belt-like straps at both ends: Just below the wrist, and ahead of the elbow.
   
   The wrist strap has a small handle that retracts or detracts the blades when it's pulled or pushed back, using 2 thin strips of white rope tied to the handle which pass through each row of blades, which will force the blades into vertical positions, or pull them back down, using knots that'll get caught on the holes which the rest of the ropes pass through between each row _(for every other row that retracts away from the handle, the ropes loop back around at the other end of the leather base before passing through them, with a slight offset to avoid tangling with itself.)_
   
   Most of the ropes are hidden inside the leather base, only visible when it pokes out and passes through small holes drilled into each row before going back into the base, in a stitch-like manner.
   
   This is shown through closeup shots when Ruiz detracts the blades, with the strips of ropes pulling forwards as Ruiz pulls the wrist handle, which has a faint bump in its strap, with the lack of rope on their other ends forcing each row of blades into vertical positions.
   
   Ruiz uses the tip of one of the blades to create stress on the front door's window, shattering it into pieces, then uses the Arm Blades as a convenient object to clear out some of the leftover glass so that he can safely reach his arm through and pull the door open from inside - the ease of which Ruiz figures would normally be cancelled out by an alarm, however the electrical grid had _almost entirely_ collapsed by that point, so it wouldn't have triggered regardless.
   
   Ruiz then retracts the blades by pushing the handle back into the wrist strap, returning the excess length of rope to their other end and letting the blades fall back into place, except for the 2nd row - the last row before the ropes end - which slightly juts upward even when the handle is fully pushed back, forcing Ruiz to manually push it down once he notices.
   
   Ruiz reaches through the broken window and pulls it open using its indoor handle, which temporary unlocks the door as countermeasure in the event that someone was locked inside, giving him and rest of SST 1 enough time to enter the store.
  
  Once Ruiz enters the store, the rest of the group follows after him.
  
  _Everett_, a man with medium brown skin whose carrying a large high illumination lantern, enters the store after Ruiz and uses the lantern to illuminate a dark aisle blocked from the already sparce sunlight by the shelfs in a bright blueish light, allowing the others to immediately begin looting for supplies in the store which has gone untouched for years.
  
  SST 1 is shown to be an experienced, coordinated team as they locate and strategically loot certain nutritious canned and non-perishable food, taking into account the number of calories, nutritional value, and compact size compared to other food products—Primarily Dave, who has memorized the details of common food products to the point that he's able to quickly pick out specific products they should take.
  
  SST 1 also restocks on any important and/or useful survival supplies that they've depleted.
  
  _SCENE: STORE_
   Though the store has large exterior windows, they face towards the _south_, and the fact the Sun is already visibly setting makes the areas just beyond the entrance pitch dark, signaling Everett to turn on the lantern he's been carrying to illuminate much of the store, allowing SST 1 to easily begin looting, which they immediately do as if they had already planned to do so.
   
   The store's interior, albeit full of dusty products and surfaces, with baskets along the storefront windows reeking with the smell of rotten produce, and expired refrigerated goods packed inside a line of non-functional freezers at the back, has seemingly gone untouched.
   
   There's a dozen handheld red shopping baskets stacked up near the entrance, and a single checkout behind a wooden counter a couple metres ahead of the entrance, and small shelfs along the adjacent wall stocked with usual last-minute shopping items such as candy, gum, birthday/anniversary cards, and magazines that seem to cover a range of generic topics _(however their covers are unreadable as they're written in **Rossgivian**, a fictional writing system used in Rossgivo which resembles Kanji, but uses unique symbols.)_
  
   The store is fairly claustrophobic, with the shelfs spaced apart just enough for two people to pass by each other, with the only significant open space being near the entrance and checkout.
  
   The store's products are normal and commonplace, such as canned food, silverware, beverages, batteries, school supplies, and toilet paper. The most notable products are at the heads of the aisles near the entrance, where there's sections of fireworks, sparklers, and festive items themed around the nation's 1000 year anniversary, further emphasising the close relation between the 1000 year anniversary and whatever apocalyptic event had occurred afterwards.
   
   As SST 1 searches through the shelfs, they take note of what they find, pass it onto others, focusing on food with high nutrition and calories, as well as looting any useful survival supplies such as batteries. There's a clear sense of coordination, familiarity, and experience among them.
  
   Ruiz tells Frank to look in the back of the store for a storage room to see if there were any additional supplies that weren't stocked on the shelfs, considering that some products could've been stored away to make room for the 1000th anniversary-themed products displayed at the head of the aisles near the entrance.
   
   Frank grabs a flashlight out from his hiking backpack to light the way as he moves into the back of the store where a storage room might exist.
   
   We get a good look at Ruiz: he's wearing a large hiking backpack full of supplies and equipment, with 2 smoke grenades stored in its right side pocket for quick access to distract Krushers with.
  
   Everyone else wears a mix of casual long-sleeved shirts, closed-toe shoes/boots, and they also have hiking backpacks full with supplies like Ruiz, though with a visible amount of empty space which gets used to store the additional supplies.
